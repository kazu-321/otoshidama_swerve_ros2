    ...
    void visualize_swerve::visualize_swerve_callback(const canable_msgs::msg::Can::SharedPtr msg) {
        ...
        if(0x100 < msg->id && msg->id < 0x105) {
            ...
            for(int i = 0; i < 4; i++) {
                ...
                text.pose.position.x = wheel_positions[marker.id][0];
                text.pose.position.y = wheel_positions[marker.id][1];
                ...
                text.scale.z = 0.05;
                text.text = std::to_string(angle[i]/M_PI) + "pi";
                if(abs(angle[i] - last_angle[i]) > M_PI) {
                    text.color.r = 1.0;
                    text.color.g = 0.0;
                    text.color.b = 0.0;
                    text.color.a = 1.0;
